...
export async function mountCalculator(root,session,{request,onSaved,onClean,canRefresh}){
  ...
  function heroHtml(){
    const view=heroView();
    if(!signedIn)return '<section class="calc-hero is-guest" data-calc-hero aria-label="This count"><div class="calc-readout"><span class="calc-label">This count</span><strong class="calc-total'+(view.total?'':' is-zero')+'" data-finance-total data-cents="'+view.total+'">'+money(view.total)+'</strong><div class="calc-bar" data-calc-bar aria-hidden="true">'+barHtml(view)+'</div><p class="calc-breakdown" data-calc-breakdown>'+breakdownHtml(view)+'</p></div>'+(standalone?'':'<div class="calc-guest-pitch"><strong>Keep a running total</strong><p>Make a free account to save counts, cash out, and see your history on any device.</p><div><button type="button" class="button primary" data-action="register">Create account</button><button type="button" class="text-button" data-action="login">Sign in</button></div></div>')+'</section>';
  ...
  function countHtml(){
    ...
    const saveButton=signedIn?'<button class="button primary" data-calc-save '+(total?'':'disabled')+'>Save count</button>':standalone?'':'<button type="button" class="button primary" data-action="register">Create account to save</button>';
    const status=hasDraft()?draftNote():signedIn?'Saving adds this count to today, this week, and all time. Enter moves to the next band, Ctrl+Enter saves.':standalone?'Enter moves to the next band. Everything stays on this device.':'Enter moves to the next band. Nothing leaves this device until you make an account.';
  ...
  function ownPage(){
    ...
    return '<div class="calc-page'+enter+'"><div class="page-heading"><div><span class="eyebrow">Calculator</span><h1>What are your bands worth?</h1><p>'+(signedIn?'Count your bands, save the total, keep a running tally.':standalone?'Count your bands or paste a screenshot of your inventory to see what they are worth.':'Count your bands and see what they are worth. No account needed.')+'</p></div>'+(standalone?'':arrangeBar)+'</div>'+heroHtml()+'<div class="calc-workspace">'+countHtml()+'<div class="calc-side">'+scanHtml()+mathHtml()+recentHtml()+'</div></div></div>';
  }
