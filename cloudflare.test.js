...
function worker(){
  const storage=fakeStorage(),api=createApi({storage:durableStorage(storage),key:randomBytes(32),local:false,cookieName:SESSION_COOKIE,hash:WORKER_HASH,setupCode:'open-sesame-42'});
...
test('the Pages site can set up, sign in and save through the Worker',async()=>{
  ...
  assert.equal((await call('/api/auth/register',{method:'POST',body:owner})).status,409);
  ...
  assert.match(db.prepare('SELECT password FROM users').get().password,/^s4096\.8\.1\$[0-9a-f]{32}:[0-9a-f]{128}$/);
