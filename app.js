...
function render() {
  ...
  if($('#finance-panel'))mountFinance($('#finance-panel'),authSession,{request:authRequest,canRefresh:canApplyRemote,onClean:()=>{pageDirty=false;},onSaved:(kind)=>{pageDirty=false;toast(kind==='removed'?'Count removed.':kind==='paid'?'Marked as paid out.':kind==='undone'?'Payout undone.':'Count saved.');}}).then(()=>{const receipt=new URLSearchParams(location.hash.split('?')[1]||'').get('receipt');if(receipt)$('#finance-panel')?.openReceipt?.(receipt);finishPage();});
...
let pollFailures=0;
// A hidden tab does not need fresh data, so it only idles; the visibilitychange listener above refreshes it on return.
async function schedulePoll(){
  const awake=document.visibilityState==='visible';
  if(awake)await pollSession();
  setTimeout(schedulePoll,awake?Math.min(30000,3000*2**pollFailures)+Math.floor(Math.random()*500):30000);
}
