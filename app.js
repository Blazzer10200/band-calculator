...
function render() {
  ...
  if($('#finance-panel'))mountFinance($('#finance-panel'),authSession,{request:authRequest,canRefresh:canApplyRemote,onClean:()=>{pageDirty=false;},onSaved:(kind)=>{pageDirty=false;toast(kind==='removed'?'Count removed.':kind==='paid'?'Marked as paid out.':'Count saved.');}}).then(()=>{const receipt=new URLSearchParams(location.hash.split('?')[1]||'').get('receipt');if(receipt)$('#finance-panel')?.openReceipt?.(receipt);finishPage();});
